...
OUTPUT_DIR="${OUTPUT_DIR:-${ROOT_DIR}/outputs/ocm_dar_suzuki_3x40_10init_no_pr_no_pure_autobo_no_ensemble}"
TASK_NAME_OVERRIDE="${TASK_NAME:-ocm_dar_suzuki_3x40_10init_no_pr_no_pure_autobo_no_ensemble}"
...
DAR_PROBLEM_FILE="${3:-$DEFAULT_DAR_PROBLEM}"
DAR_CONFIG_FILE="${4:-$DEFAULT_DAR_CONFIG}"
SUZUKI_PROBLEM_FILE="${5:-$DEFAULT_SUZUKI_PROBLEM}"
SUZUKI_CONFIG_FILE="${6:-$DEFAULT_SUZUKI_CONFIG}"
...
dar_problem_path = Path(sys.argv[4]).resolve()
dar_config_path = Path(sys.argv[5]).resolve()
suzuki_problem_path = Path(sys.argv[6]).resolve()
suzuki_config_path = Path(sys.argv[7]).resolve()
