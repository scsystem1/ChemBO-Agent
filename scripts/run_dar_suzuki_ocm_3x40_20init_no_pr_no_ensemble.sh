...
DEFAULT_SUZUKI_PROBLEM="${ROOT_DIR}/examples/suzuki_problem.yaml"
DEFAULT_SUZUKI_CONFIG="${ROOT_DIR}/dashscope_kimi.yaml"
...
OUTPUT_DIR="${OUTPUT_DIR:-${ROOT_DIR}/outputs/dar_suzuki_ocm_3x40_20init_no_pr_no_ensemble}"
TASK_NAME_OVERRIDE="${TASK_NAME:-dar_suzuki_ocm_3x40_20init_no_pr_no_ensemble}"
...
SUZUKI_PROBLEM_FILE="${3:-$DEFAULT_SUZUKI_PROBLEM}"
SUZUKI_CONFIG_FILE="${4:-$DEFAULT_SUZUKI_CONFIG}"
OCM_PROBLEM_FILE="${5:-$DEFAULT_OCM_PROBLEM}"
OCM_CONFIG_FILE="${6:-$DEFAULT_OCM_CONFIG}"
...
suzuki_problem_path = Path(sys.argv[4]).resolve()
suzuki_config_path = Path(sys.argv[5]).resolve()
ocm_problem_path = Path(sys.argv[6]).resolve()
ocm_config_path = Path(sys.argv[7]).resolve()
output_dir = Path(sys.argv[8]).resolve()
task_name_override = sys.argv[9].strip()
repeats = int(sys.argv[10])
budget = int(sys.argv[11])
warm_start = int(sys.argv[12])
summary_json_path = Path(sys.argv[13]).resolve()
summary_csv_path = Path(sys.argv[14]).resolve()
...
def _run_domain(
    *,
    ...
    if not isinstance(base_problem, dict):
        raise RuntimeError(f"{domain.upper()} batch script expects a structured YAML/JSON problem file.")
    ...
    for run_index in range(1, repeats + 1):
        ...
        print("============================================================")
        print(f"{domain.upper()} repeat {run_index}/{repeats}")
        ...
        print("Settings override: pure_reasoning_ablation_enabled=False, ensemble_af=False")
        print("============================================================")
...
dar_output_dir = output_dir / "dar"
suzuki_output_dir = output_dir / "suzuki"
ocm_output_dir = output_dir / "ocm"

dar_output_dir.mkdir(parents=True, exist_ok=True)
suzuki_output_dir.mkdir(parents=True, exist_ok=True)
ocm_output_dir.mkdir(parents=True, exist_ok=True)
...
all_summaries.extend(
    _run_domain(
        ...
        domain_output_dir=dar_output_dir,
        experiment_name=f"{task_name_override}_dar",
    )
)
all_summaries.extend(
    _run_domain(
        domain="suzuki",
        problem_path=suzuki_problem_path,
        config_path=suzuki_config_path,
        domain_output_dir=suzuki_output_dir,
        experiment_name=f"{task_name_override}_suzuki",
    )
)
all_summaries.extend(
    _run_domain(
        ...
        domain_output_dir=ocm_output_dir,
        experiment_name=f"{task_name_override}_ocm",
    )
)
