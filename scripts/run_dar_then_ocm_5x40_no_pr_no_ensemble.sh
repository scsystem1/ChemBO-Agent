...
REPEATS="${REPEATS:-5}"
DAR_BUDGET="${DAR_BUDGET:-40}"
OCM_BUDGET="${OCM_BUDGET:-40}"
OUTPUT_DIR="${OUTPUT_DIR:-${ROOT_DIR}/outputs/dar_then_ocm_5x40_no_pr_no_ensemble}"
TASK_NAME_OVERRIDE="${TASK_NAME:-dar_then_ocm_5x40_no_pr_no_ensemble}"
...
"${PYTHON_CMD[@]}" - "${ROOT_DIR}" "${DAR_PROBLEM_FILE}" "${DAR_CONFIG_FILE}" "${OCM_PROBLEM_FILE}" "${OCM_CONFIG_FILE}" "${OUTPUT_DIR}" "${TASK_NAME_OVERRIDE}" "${REPEATS}" "${DAR_BUDGET}" "${OCM_BUDGET}" "${SUMMARY_JSON}" "${SUMMARY_CSV}" <<'PY'
...
dar_budget = int(sys.argv[9])
ocm_budget = int(sys.argv[10])
summary_json_path = Path(sys.argv[11]).resolve()
summary_csv_path = Path(sys.argv[12]).resolve()
...
def _run_domain(
    *,
    ...
    for run_index in range(1, repeats + 1):
        ...
        settings.initial_doe_size = initial_doe_size
