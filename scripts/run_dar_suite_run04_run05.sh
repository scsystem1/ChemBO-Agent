...
MAX_CPU_CORES="${MAX_CPU_CORES:-40}"
BASE_SEED="${BASE_SEED:-20260423}"
SEED_STRIDE="${SEED_STRIDE:-1000}"

CHEMBO_OUTPUT_DIR="${CHEMBO_OUTPUT_DIR:-${ROOT_DIR}/outputs/dar_3x_40iter}"
CHEMBO_TASK_NAME="${CHEMBO_TASK_NAME:-dar_3x_40iter}"
REASONING_OUTPUT_DIR="${REASONING_OUTPUT_DIR:-${ROOT_DIR}/outputs/baseline_runs/reasoning_bo/dar}"
GOLLUM_OUTPUT_DIR="${GOLLUM_OUTPUT_DIR:-${ROOT_DIR}/outputs/baseline_runs/gollum/dar}"
...
echo "[DAR Suite] Running ChemBO, Reasoning-BO, GOLLuM, and PrefBO for run$(printf "%02d" "${START_RUN_INDEX}")-run$(printf "%02d" "${END_RUN_INDEX}")"
echo "[DAR Suite] CPU thread cap: ${CPU_THREAD_CAP}"
...
for run_index in $(seq "${START_RUN_INDEX}" "${END_RUN_INDEX}"); do
  ...
  echo "[ChemBO][DAR] Starting ${run_id} with seed=${run_seed}"
...
if not isinstance(base_problem, dict):
    raise RuntimeError("DAR suite script expects a structured YAML/JSON problem file.")

run_id = f"run{run_index:02d}"

print("============================================================")
print(f"DAR repeat {run_index}")
...
echo "[ReasoningBO][DAR] Appending trials ${START_RUN_INDEX}-${END_RUN_INDEX}"
...
echo "[GOLLuM][DAR] Appending trials ${START_RUN_INDEX}-${END_RUN_INDEX}"
...
echo "[PrefBO][DAR] Appending trials ${START_RUN_INDEX}-${END_RUN_INDEX}"
...
echo "[DAR Suite] Completed run$(printf "%02d" "${START_RUN_INDEX}")-run$(printf "%02d" "${END_RUN_INDEX}")"
