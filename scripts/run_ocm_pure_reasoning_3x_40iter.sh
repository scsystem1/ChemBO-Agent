...
REPEATS="${REPEATS:-3}"
BUDGET="${BUDGET:-40}"
OUTPUT_DIR="${OUTPUT_DIR:-${ROOT_DIR}/outputs/ocm_pure_reasoning_3x_40iter}"
TASK_NAME_OVERRIDE="${TASK_NAME:-ocm_pure_reasoning_3x_40iter}"
...
if not isinstance(base_problem, dict):
    raise RuntimeError("OCM pure reasoning batch script expects a structured YAML/JSON problem file.")
...
for run_index in range(1, repeats + 1):
    ...
    print("============================================================")
    print(f"OCM pure reasoning repeat {run_index}/{repeats}")
