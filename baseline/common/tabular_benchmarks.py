from __future__ import annotations

from dataclasses import dataclass
from pathlib import Path
from typing import Iterable

import pandas as pd

OCM_NON_VARIABLE_COLUMNS = {"Name", "M1_mol", "M2_mol", "M3_mol"}


@dataclass(frozen=True)
class BenchmarkSpec:
    name: str
    data_path: Path
    target_column: str
    feature_columns: list[str]


def _drop_index_like_columns(df: pd.DataFrame) -> pd.DataFrame:
    drop_cols = [col for col in df.columns if str(col).startswith("Unnamed:")]
    if "" in df.columns:
        drop_cols.append("")
    if drop_cols:
        df = df.drop(columns=drop_cols)
    return df


def load_benchmark_spec(root: Path, dataset_name: str) -> tuple[BenchmarkSpec, pd.DataFrame]:
    dataset_key = dataset_name.strip().lower()
    if dataset_key == "dar":
        path = root / "data" / "DAR.csv"
        target_column = "yield"
    elif dataset_key == "ocm":
        path = root / "data" / "OCM.csv"
        target_column = "Performance"
    elif dataset_key == "suzuki":
        path = root / "data" / "suzuki.csv"
        target_column = "Product_Yield_PCT_Area_UV"
    elif dataset_key == "oer":
        path = root / "data" / "OER.csv"
        target_column = "objective"
    elif dataset_key == "hpobench_svm":
        path = root / "data" / "HPOBench" / "hpobench_svm_146212.csv"
        target_column = "test_acc"
    elif dataset_key == "hpobench_rf":
        path = root / "data" / "HPOBench" / "hpobench_rf_146606.csv"
        target_column = "test_acc"
    elif dataset_key == "hpobench_xgb":
        path = root / "data" / "HPOBench" / "hpobench_xgb_146606.csv"
        target_column = "test_acc"
    elif dataset_key == "hpobench_nn":
        path = root / "data" / "HPOBench" / "hpobench_nn_168912.csv"
        target_column = "test_acc"
    else:
        raise ValueError(f"Unsupported dataset: {dataset_name}")

    df = pd.read_csv(path)
    df = _drop_index_like_columns(df)
    if dataset_key == "ocm":
        feature_columns = [
            col for col in df.columns if col != target_column and col not in OCM_NON_VARIABLE_COLUMNS
        ]
    elif dataset_key == "oer":
        feature_columns = [
            col for col in df.columns if col not in {target_column, "procedure"}
        ]
    elif dataset_key.startswith("hpobench_"):
        feature_columns = [
            col
            for col in df.columns
            if col not in {target_column, "test_loss", "seed_count", "seeds", "row_id"}
        ]
    else:
        feature_columns = [col for col in df.columns if col != target_column]
    spec = BenchmarkSpec(
        name=dataset_key,
        data_path=path,
        target_column=target_column,
        feature_columns=feature_columns,
    )
    return spec, df


def dataset_row_to_text(dataset_name: str, row: pd.Series) -> str:
    dataset_key = dataset_name.strip().lower()
    if dataset_key == "dar":
        return (
            "Direct arylation reaction with "
            f"base {row['base_SMILES']}, "
            f"ligand {row['ligand_SMILES']}, "
            f"solvent {row['solvent_SMILES']}, "
            f"concentration {row['concentration']} M, "
            f"temperature {row['temperature']} C."
        )
    if dataset_key == "ocm":
        return (
            "Oxidative coupling of methane catalyst and condition with "
            f"catalyst {row['Name']}, "
            f"metals {row['M1']}/{row['M2']}/{row['M3']}, "
            f"support {row['Support']}, "
            f"metal mol fractions {row['M1_mol']}/{row['M2_mol']}/{row['M3_mol']}, "
            f"temperature {row['Temp']} C, "
            f"Ar flow {row['Ar_flow']}, "
            f"CH4 flow {row['CH4_flow']}, "
            f"O2 flow {row['O2_flow']}, "
            f"contact time {row['CT']}."
        )
    if dataset_key == "suzuki":
        return (
            "Suzuki-Miyaura coupling with "
            f"aryl halide {row['Reactant_1_Name']}, "
            f"boronic acid partner {row['Reactant_2_Name']}, "
            f"ligand {row['Ligand_Short_Hand']}, "
            f"base {row['Reagent_1_Short_Hand']}, "
            f"solvent {row['Solvent_1_Short_Hand']}."
        )
    if dataset_key == "oer":
        if "procedure" in row and pd.notna(row["procedure"]):
            return str(row["procedure"])
        return (
            "OER catalyst composition with "
            f"nickel loading {row['ni_load']}, "
            f"iron loading {row['fe_load']}, "
            f"cobalt loading {row['co_load']}, "
            f"manganese loading {row['mn_load']}, "
            f"cerium loading {row['ce_load']}, "
            f"lanthanum loading {row['la_load']}."
        )
    if dataset_key.startswith("hpobench_"):
        feature_columns = [
            col
            for col in row.index
            if col not in {"test_acc", "test_loss", "seed_count", "seeds", "row_id"}
        ]
        assignments = ", ".join(f"{col}={row[col]}" for col in feature_columns)
        model = dataset_key.removeprefix("hpobench_").upper()
        return f"HPOBench {model} configuration with {assignments}."
    raise ValueError(f"Unsupported dataset: {dataset_name}")


def dataframe_to_texts(dataset_name: str, df: pd.DataFrame) -> list[str]:
    return [dataset_row_to_text(dataset_name, row) for _, row in df.iterrows()]


def dataframe_to_one_hot(df: pd.DataFrame, feature_columns: Iterable[str]) -> pd.DataFrame:
    feature_df = df.loc[:, list(feature_columns)].copy()
    categorical_cols = [
        col for col in feature_df.columns if feature_df[col].dtype == "object"
    ]
    if categorical_cols:
        feature_df = pd.get_dummies(feature_df, columns=categorical_cols)
    return feature_df
