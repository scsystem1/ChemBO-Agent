...
def dataset_templates(dataset_name: str) -> tuple[str, str, str]:
    if dataset_name == "hpobench_svm":
        return (
            "HPOBench SVM Hyperparameter Optimization",
            "Machine Learning/Hyperparameter Optimization",
            "Optimize HPOBench SVM test accuracy on OpenML task 146212 using exact grid configurations for C and gamma at maximum fidelity.",
        )
    if dataset_name == "hpobench_rf":
        return (
            "HPOBench Random Forest Hyperparameter Optimization",
            "Machine Learning/Hyperparameter Optimization",
            "Optimize HPOBench random forest test accuracy on OpenML task 146606 using exact grid configurations at maximum fidelity.",
        )
    if dataset_name == "hpobench_xgb":
        return (
            "HPOBench XGBoost Hyperparameter Optimization",
            "Machine Learning/Hyperparameter Optimization",
            "Optimize HPOBench XGBoost test accuracy on OpenML task 146606 using exact grid configurations at maximum fidelity.",
        )
    if dataset_name == "hpobench_nn":
        return (
            "HPOBench Neural Network Hyperparameter Optimization",
            "Machine Learning/Hyperparameter Optimization",
            "Optimize HPOBench neural network test accuracy on OpenML task 168912 using exact grid configurations at maximum fidelity.",
        )
    if dataset_name == "dar":
        return (
    ...
    if dataset_name == "suzuki":
        return (
            "Suzuki-Miyaura Coupling Optimization",
            "Chemistry/Cross-Coupling Reaction Optimization",
            "Optimize Suzuki-Miyaura coupling yield by selecting reactants, ligand, base, and solvent combinations from a tabulated reaction design space.",
        )
    if dataset_name == "ocm":
        return (
    ...
    if dataset_name == "oer":
        return (
            "Oxygen Evolution Reaction Catalyst Optimization",
            "Chemistry/Electrocatalyst Composition Optimization",
            "Optimize oxygen evolution reaction catalyst performance by selecting nickel, iron, cobalt, manganese, cerium, and lanthanum loadings from a tabulated catalyst composition design space.",
        )
...
def build_search_space_and_config(
    ...
    for column in feature_columns:
        ...
        parameters.append(
            ChoiceParameter(
                ...
                values=bounds,
                is_ordered=parameter_type != ParameterType.STRING,
                sort_values=parameter_type != ParameterType.STRING,
            )
        )
    ...
    config_payload = {
        ...
        "constraint": "All parameter values must come exactly from the predefined design space; for HPOBench tasks, do not interpolate between grid values.",
...
def run_trial(
    ...
    if reasoner_name == "deepseek":
        reasoner = DSReasoner(exp_config_path=str(config_path), result_dir=str(output_dir))
    elif reasoner_name == "kimi":
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Run Reasoning-BO on tabular chemistry or HPOBench datasets.")
    parser.add_argument("--dataset", choices=supported_datasets, required=True)
    parser.add_argument("--reasoner", choices=["kimi", "deepseek"], default=os.getenv("REASONINGBO_REASONER", "kimi"))
    ...
    if args.reasoner == "deepseek":
        if not os.getenv("DEEPSEEK_API_KEY"):
            raise RuntimeError("Missing DeepSeek API key. Set DEEPSEEK_API_KEY.")
        os.environ.setdefault("DEEPSEEK_API_BASE", "https://api.deepseek.com")
        os.environ.setdefault("DEEPSEEK_MODEL_NAME", "deepseek-v4-pro")
    elif not (
    ...
    summary = {
        ...
        "llm_model": (
            os.getenv("DEEPSEEK_MODEL_NAME", "deepseek-v4-pro")
            if args.reasoner == "deepseek"
            else os.getenv("REASONINGBO_LLM_MODEL", os.getenv("KIMI_MODEL", "kimi-k2.5-thinking"))
        ),
    }
