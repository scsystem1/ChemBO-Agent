...
class WeightedIndicatorKernel(gpytorch.kernels.Kernel):
    ...
    def __init__(self, cat_dims: list[int], init_weights: list[float] | None = None, **kwargs: Any):
        ...
        self.register_parameter(
            "raw_weights",
            torch.nn.Parameter(self._weight_constraint.inverse_transform(init_tensor)),
        )
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if not self.cat_dims:
            ...
            self.register_constraint("raw_weights", self._weight_constraint)
        gamma_tensor = torch.as_tensor(float(max(init_gamma, 1e-6)), dtype=torch.get_default_dtype()).reshape(1)
        self.register_parameter(
            "raw_gamma",
            torch.nn.Parameter(self._gamma_constraint.inverse_transform(gamma_tensor)),
        )
...
class LatentCategoricalKernel(gpytorch.kernels.Kernel):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        lengthscale = torch.as_tensor(float(max(init_lengthscale, 1e-6)), dtype=torch.get_default_dtype()).reshape(1)
        self.register_parameter(
            "raw_lengthscale",
            torch.nn.Parameter(self._lengthscale_constraint.inverse_transform(lengthscale)),
        )
    ...
    def forward(
        ...
        **params: Any,
        ...
        for spec, dim, embedding_table in zip(self.cat_specs, self.cat_dims, self.embeddings):
            ...
            encoding_scale = float(spec.get("encoding_scale", 1.0) or 1.0)
            idx1 = torch.round(x1[..., dim] * encoding_scale).long().clamp_min(0).clamp_max(n_categories - 1)
            idx2 = torch.round(x2[..., dim] * encoding_scale).long().clamp_min(0).clamp_max(n_categories - 1)
